...
def register_routes(app, create_db_connection, make_api_request):
    @app.route('/')
    def index():
    ...
    def get_processes():
        ...
        agent_url = f"http://{session['selected_agent']}:{app.config['AGENT_PORT']}/processes"
        
        try:
            response = requests.get(agent_url, headers={'X-API-Key': app.config['API_KEY']}, timeout=5)
